...
#import "SporadicM24ViewController.h"
...
@implementation SporadicM24View
...
- (void) awakeFromNib
{
    ...
    CalculateBallCoordinates( circleCenter, circleRadius, M24BallRadius, ballCoordinates, tagCoordinates);   //fills array with coordinates for the center of the ball
    CGRect frame = CGRectMake ( 0.0, 0.0, M24BallRadius*2, M24BallRadius*2 );
...
- (bool) recordTouches: (NSSet *)touches
{
    UITouch * touch = [ touches anyObject ];
	if ( ! touch )
		return false;
...
- (void) finishSwapWithTouches: (NSSet *)touches
{
	UITouch * touch = [ touches anyObject ];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *) event {
    ...
    {
		[ self finishSwapWithTouches: touches ];
		return;
    }
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *) event {
    ...
    if ( firstWedgeTouched == - 1 )
    {
		// How did we somehow get touchesMoved without touchesBegan?
        [ self touchesBegan:touches withEvent:event ] ;
        return;
    }

    if ( [ self recordTouches:touches ] )
		return;
