...
bool test( const Perm & p, M24PermInfoTable & table  )
{
  M24PermutationWithHistory pinv = table.lookup_shortest_inverse( p );
...
typedef bool testfn( M24PermInfoTable & table );

bool test_RSRS( M24PermInfoTable & table )
{
  return test( Perm( ).right( ).swap( ).right( ).swap( ),
               table );
}

bool test_bug1( M24PermInfoTable & table )
{
  Perm::PermArray p={ 15, 6, 9, 16, 0, 7, 18, 5, 14, 3, 8, 19, 1, 12, 23, 20, 11, 4, 17, 10, 13, 22, 21, 2 };
  return test( Perm( p ), table );
}



bool test_R7SR9SR7( M24PermInfoTable & table )
{
  return test( Perm( ).right( 7 ).swap( ).right( 9 ).swap( ).right( 7 ),
               table );
}

bool test_one_random( bool amnesia, M24PermInfoTable & table  )
{
  M24PermutationWithHistory p;
  p.random( amnesia );
...
bool test_random( M24PermInfoTable & table )
{
  bool result = true;
  bool amnesia = true;
...
int run_tests( M24PermInfoTable & table )
{
  int result = 0;
...
int usage( char *argv[], int return_code )
{
  ...
  os << "usage: " << argv[ 0 ] << " [OPTION]... [FILE]" << endl
     << "Test M24Permutation lookup" << endl
